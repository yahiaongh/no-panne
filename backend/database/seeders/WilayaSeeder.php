...
{
    /**
     * The 48 Algerian wilayas (codes + official French names).
     *
     * Coordinates are approximate; the full administrative dataset
     * (communes, official Arabic names, exact centroids) is imported
     * separately — see docs/backend-implementation-plan.md.
     */
    ...
    public function run(): void
    {
        foreach ($this->wilayas as [$code, $name]) {
            Wilaya::query()->updateOrCreate(
                ['code' => str_pad((string) $code, 2, '0', STR_PAD_LEFT)],
                ['name_fr' => $name, 'active' => true]
            );
        }
    }
}